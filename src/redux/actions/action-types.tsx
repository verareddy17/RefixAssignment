export const LOAD_RESOURCE_START = 'LOAD_RESOURCE_START';
export const LOAD_RESOURCE_SUCCESS = 'LOAD_RESOURCE_SUCCESS';
export const LOAD_RESOURCE_FAIL = 'LOAD_RESOURCE_FAIL';
export const LOAD_USER_START = 'LOAD_USER_START';
export const LOAD_USER_SUCCESS = 'LOAD_USER_SUCCESS';
export const LOAD_USER_FAILURE = 'LOAD_USER_FAILURE';
export const INPUT_TEXT = 'INPUT_TEXT';
export const RESET = 'RESET';
export const DOWNLOAD_START = 'DOWNLOAD_START';
export const DOWNLOAD_PROGRESS = 'DOWNLOAD_PROGRESS';
export const DOWNLOAD_SUCCESS = 'DOWNLOAD_SUCCESS';
export const DOWNLOAD_FAILURE = 'DOWNLOAD_FAILURE';
export const LOAD_SETTINGS_START = 'LOAD_SETTINGS_START';
export const LOAD_SETTINGS_SUCCESS = 'LOAD_SETTINGS_SUCCESS';
export const LOAD_SETTINGS_FAIL = 'LOAD_SETTINGS_FAILURE';
export const ADD_DOWNLOADED_FILE = 'ADD_DOWNLOADED_FILE';
export const REMOVE_DOWNLOADED_FILE = 'REMOVE_DOWNLOADED_FILE';
export const ADD_BOOKMARK_FILE = 'ADD_BOOKMARK_FILE';
export const REMOVE_BOOKMARK_FILE = 'REMOVE_BOOKMARK_FILE';
export const FETCH_ALL_DOWNLOADED_FILES = 'FETCH_ALL_DOWNLOADED_FILES';
export const LOAD_SEARCH_FILES = 'LOAD_SEARCH_FILES';
export const LOAD_DOWNLOADED_FILE_START = 'LOAD_DOWNLOADED_FILE_START';
